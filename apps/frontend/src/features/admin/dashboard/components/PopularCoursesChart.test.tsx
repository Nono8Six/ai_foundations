...
import { supabase } from '@core/supabase/client'; // Mocked
...
describe('PopularCoursesChart', () => {
  ...
  test('displays loading state and then renders chart with processed data', async () => {
    const courses = [
      {
        ...
        title: 'Course A',
        is_published: true,
        modules: [{ id: 10, lessons: [{ id: 101 }, { id: 102 }] }],
      },
      {
        id: 2,
        title: 'Course B',
        is_published: true,
        modules: [{ id: 20, lessons: [{ id: 201 }] }],
      },
      {
        id: 3,
        title: 'Course C (Not Published)',
        is_published: false,
        modules: [{ id: 30, lessons: [{ id: 301 }] }],
      },
      {
        id: 4,
        title: 'Course D',
        is_published: true,
        modules: [{ id: 40, lessons: [{ id: 401 }] }],
      },
    ];
    const userProgress = [
      // User 1: Enrolled in A, completed A. Enrolled B.
      { user_id: 'user1', lesson_id: 101, status: 'completed' },
      { user_id: 'user1', lesson_id: 102, status: 'completed' },
    ...
    supabase.from
      .mockReturnValueOnce(
        createBuilder({ data: courses.filter(c => c.is_published), error: null })
      )
    ...
    expect(
      screen.getByText('Total inscriptions').closest('div').querySelector('p.text-lg').textContent
    ).toBe('5');
    expect(
      screen.getByText('Total complétions').closest('div').querySelector('p.text-lg').textContent
    ).toBe('3');
    expect(
      screen.getByText('Note moyenne').closest('div').querySelector('p.text-lg').textContent
    ).toBe('N/A');
  });
  ...
  test('displays "No data available" when no published courses are fetched', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Aucune donnée disponible pour les cours populaires.')
      ).toBeInTheDocument();
    });
  });
  ...
  test('handles error when fetching user progress', async () => {
    ...
    supabase.from
      ...
      .mockReturnValueOnce(
        createBuilder({ data: null, error: { message: 'Progress fetch error' } })
      );
