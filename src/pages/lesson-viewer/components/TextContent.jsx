...
const TextContent = ({ content, onProgress }) => {
  ...
  const parseContent = (text) => {
    return text
      ...
      .map((line, index) => {
        if (line.startsWith('# ')) {
          return <h1 key={index} className="text-3xl font-bold mb-6 text-text-primary">{line.slice(2)}</h1>;
        }
        if (line.startsWith('## ')) {
          return <h2 key={index} className="text-2xl font-semibold mb-4 text-text-primary">{line.slice(3)}</h2>;
        }
        if (line.startsWith('### ')) {
          return <h3 key={index} className="text-xl font-semibold mb-3 text-text-primary">{line.slice(4)}</h3>;
        }
        if (line.startsWith('- ')) {
          return <li key={index} className="mb-2">{line.slice(2)}</li>;
        }
        if (line.match(/^\d+\./)) {
          return <li key={index} className="mb-2">{line.replace(/^\d+\.\s*/, '')}</li>;
        }
        if (line.trim() === '') {
