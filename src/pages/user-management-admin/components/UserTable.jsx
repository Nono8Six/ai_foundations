...
const UserTable = ({
  ...
  const getActivityStatus = (lastActivity) => {
    ...
    if (daysSinceActivity <= 7) return { color: 'text-warning', label: `Il y a ${daysSinceActivity}j` };
    if (daysSinceActivity <= 30) return { color: 'text-warning', label: `Il y a ${daysSinceActivity}j` };
    ...
    <div className="bg-surface rounded-lg border border-border shadow-subtle overflow-hidden">
      ...
      {users.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-text-secondary">Essayez de modifier vos critères de recherche ou filtres.</p>
        </div>
      )}
    </div>
  );
