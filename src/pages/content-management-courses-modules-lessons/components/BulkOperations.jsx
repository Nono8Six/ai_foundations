    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-surface rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
              ...
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-error-700 mb-4">
                    Cette action supprimera définitivement {selectedItems.length} élément{selectedItems.length > 1 ? 's' : ''} 
          ...
          {selectedOp && (
            <div className={`mb-6 p-4 ${selectedOp.bgColor} border ${selectedOp.borderColor} rounded-lg`}>
              <div className="flex items-start">
                <Icon 
                ...
                <div>
                  ...
                  <p className="text-sm text-text-secondary">
                    Cette action sera appliquée à {selectedItems.length} élément{selectedItems.length > 1 ? 's' : ''}.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-end space-x-3 p-6 border-t border-border bg-secondary-50">
          ...
          <button
            ...
            disabled={!selectedOperation || (requiresConfirmation && confirmationText !== 'SUPPRIMER')}
            className={`px-6 py-2 rounded-lg font-medium transition-colors duration-200 ${
              !selectedOperation || (requiresConfirmation && confirmationText !== 'SUPPRIMER')
                ? 'bg-secondary-300 text-secondary-500 cursor-not-allowed'
                : selectedOp?.id === 'delete' ?'bg-error text-white hover:bg-error-600' :'bg-primary text-white hover:bg-primary-700'
            }`}
          >
            {selectedOp ? (
