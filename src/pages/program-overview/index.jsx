...
const ProgramOverview = () => {
  ...
  const formattedCourses = useMemo(() => {
    ...
    return courses.map(course => ({
      ...
      image: course.cover_image_url || 'https://images.unsplash.com/photo-1677442136019-21780ecad995?w=400&h=250&fit=crop',
  ...
  const filteredAndSortedCourses = useMemo(() => {
    let filtered = formattedCourses.filter(course => {
      ...
      const matchesSkillLevel = filters.skillLevel.length === 0 || filters.skillLevel.includes(course.difficulty);
      const matchesDuration = filters.duration.length === 0 || filters.duration.some(duration => {
      ...
      const matchesStatus = filters.status.length === 0 || filters.status.some(status => {
      ...
      return matchesSearch && matchesSkillLevel && matchesDuration && matchesCategory && matchesStatus;
    });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              <Link to="/public-homepage" className="text-text-secondary hover:text-primary transition-colors">
                Accueil
              </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-text-secondary max-w-3xl">
            Découvrez notre catalogue complet de formations en intelligence artificielle, 
            conçues pour tous les niveaux et adaptées aux besoins professionnels.
          </p>
        </div>
